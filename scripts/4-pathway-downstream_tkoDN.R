...
#make out dir
outdir <- 'results/comparative-de/TKO-vs-DKO/4.downstream/clusterprofiler/dn'
...
clustpercs[clustpercs$perc>=10,'relabel'] <- c('Cluster_1_Hypoxia', 'Cluster_2_Cholesterol_synthesis', 'Cluster_3_Mesenchymal')
...
dotplot(obj, showCategory=10, label_format=25, font.size = 9)+
  ggtitle('top pathways overall') +
  theme(plot.title = element_text(hjust = 0.5))+
  #scale_color_distiller(palette = 'Purples', direction = -1)+
...
pdf(paste0(outdir, '/finalplots_cnet.pdf'), width = 7, height = 7)
...
cnetplot(obj, showCategory = pways, foldChange = namedfc,
  ...
  ggtitle('top pathways overall') +
  theme(plot.title = element_text(hjust = 0.5))+
  scale_color_gradient(low='dodgerblue', high='gray', name='fold change')
...
mycats <- data.frame(category = rep(c( 'MSIGDB Hallmarks', 'GO: Biological Process', 'C6:Oncogenic'), each=3),
                     ID = c('HALLMARK_HYPOXIA', "HALLMARK_MTORC1_SIGNALING" , 'HALLMARK_TNFA_SIGNALING_VIA_NFKB',
                            'GOBP_RESPONSE_TO_OXYGEN_LEVELS', 'GOBP_CELLULAR_RESPONSE_TO_CHEMICAL_STRESS', 'GOBP_STEROL_METABOLIC_PROCESS',
                            "E2F1_UP.V1_DN", "MTOR_UP.N4.V1_UP" , "AKT_UP.V1_UP" )
)
